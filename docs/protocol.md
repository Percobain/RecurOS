...
boundary in RecurOS: the files on disk, the records in the log, how
...
8. Packs
9. Files RecurOS writes into your repositories
...
behind RecurOS's back), the index is discarded and rebuilt. `ctx reindex`
...
rebuilt. Upgrading RecurOS never needs a migration, because nothing in the
...
touched a file it does not own: RecurOS aborts the rebase (nothing is
...
If the user has no git identity configured, commits use `RecurOS
...
RecurOS owns only the region between
...
RecurOS and keeps working if a hook breaks. It is the floor everything
else is built on.
...
```
<!-- ctx:doc name=spec cid=b3:... branch=habit/research (generated by RecurOS; ...) -->
```
...
tell the agent to read it and run `ctx` itself for anything RecurOS does
...
*Why `.ctx/`:* everything RecurOS owns in a repository lives in one
...
*Why the marker:* it lets RecurOS tell "a file we wrote and nobody
...
RecurOS never get a failing hook.
...
The flow RecurOS is designed around: an idea starts in a chat, is
...
- **Local by default.** Nothing leaves the machine unless the user adds a
  git remote or deploys the Worker. There is no RecurOS service and no
  telemetry.
...
- **Hooks and configs** only add RecurOS's own entries and never modify
